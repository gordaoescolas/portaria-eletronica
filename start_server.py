...
app.config['TELEGRAM_TOKEN'] = 'SEU_TOKEN_AQUI'
app.config['TELEGRAM_API_URL'] = f'https://api.telegram.org/bot{app.config["TELEGRAM_TOKEN"]}/sendMessage'
app.config['COOLDOWN_MINUTES'] = 5

# Configurações da instituição para carteirinhas
CONFIG = {'escola': 'CE NOVO FUTURO', 'telefone': '61 91234-5678', 'endereco': 'Rua dos Bobos, nº 0', 'validade': '31/12/2025', 'assinatura': 'assinatura.png'}
...
def enviar_telegram(chat_id, nome, tipo_acesso):
    ...
    try:
        response = requests.post(app.config['TELEGRAM_API_URL'], params=params, timeout=5)
